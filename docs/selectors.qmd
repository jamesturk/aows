...
When you evaluate XPath against a document, you are either evaluating against the root node (`<html>`) or some already-selected child node.

If you want to search for matches anywhere in the document, you'd start your XPath with `//`.  So `//div` would select all `<div>` elements anywhere in the document.

It is also possible to start from a given node, for example `./div` would select all `<div>` elements that are children of the current node.

You can also use `..` to select the parent of the current node, so `../div` would select all `<div>` elements that are children of the parent of the current node.
...
A location step can take the form `axis::node_type[predicate]` with each of the three parts are optional.

Up until now our examples have just been using node types.  Node types are the name of a given tag, or `*` to match all elements.
...
The `[]` syntax seen when selecting by attribute is actually a very powerful tool called a predicate.
...
### A Special Note: Class Selectors
